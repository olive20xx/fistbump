    ...
    (!peers ? <p>Loading</p> :
      (peers?.map((peer, index) => (
        <div className="pt-12" key={peer._id}>
          <Popover open={open === peer._id} onOpenChange={() => setOpen(peer._id)} >
            ...
            <Button disabled={peer.reviewerId !== null || popoverStates[index].disabled} onClick={() => handleNominatePeer(index)}>Nominate</Button>
          </Popover>
        </div>
      )))
    ))
